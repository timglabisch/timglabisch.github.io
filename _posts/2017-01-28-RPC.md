...
title: "[de] Microservices Performance und RPC"
---

# Microservices Performance und RPC
...
Schaut man sich an, was z.b. durch das Google / Facebook Netzwerk fließen muss, so wird man schnell über GRPC und Thrift stolpern.
RPC scheint dort seit vielen Jahren gut zu funktionieren, in klassischen Webanwendungen findet man es aber kaum anzutreffen.
...
Der Benefit zu den "klassischen Rest Webservices" (s.o.) müsste in unserem Falle somit gegen 0 gehen.

Nachdem ich kurzerhand nichts passendes gefunden hatte, hab ein Prototyp für einen RPC Server geschrieben.
...
Der RPC Request findest in einem klassischen Webrequest statt.
...
An dem RPC Server sind Worker registriert, welche die Nachricht entgegen nehmen, verarbeiten und eine Antwort senden.
...
Als Kommunikationsformat bieten sich z.b. [Protobuf](https://developers.google.com/protocol-buffers/) an.
...
Ich habe zuhause 3 Rechner direkt mit einem Gigabit Switch verbunden, mit denen ich primär Bastel. Die Rechner habenjeweils einem AMD Phenom(tm) II X4 955 Processor und 16GB DDR3 Ram.

Lässt man die Skripte dort unter PHP 7.0.15-1~dotdeb+8.1 (cli) ( NTS )
...
Der Ramverbrauch war dabei konstant niedrig (Htop spach von 0,3%).

Selbst ohne jegliche Optimierung mit einem in PHP geschriebenen RPC Server lässen sich auf den Rechnern somit **über 20.000 RPC Reqests pro Sekunde** beantworten.

Die aktuelleste Version ist etwas langsamer, letztlich geht es für den Moment aber auch mehr darum um, mit der Implementation zu spielen um zu erarbeiten, wie ein RPC-Server für die PHP-Welt aussehen müsste.

## Next Steps

20.000 RPC Requests pro Sekunde sind schon ganz nett, es ging mir jedoch in erster Linie darum einen ersten Proof zu haben, in wie weit es realistisch sein könnte mit wirklichen vielen kleinen Microservices agieren zu können.

Für mich ergibt sich nun das Bild, dass selbst bei einer performancetechnisch schwachen PHP Implementation dies möglich sein könnte.
...
Derzeit werden schlicht Bytes ausgetauscht, in der Paxis möchte ich es möglichst einfach machen, Protobuf Messages zu verschicken. Um die Hürde so einfach zu gestalten wie nur möglich bietet sich zudem JSON als Austauschformat an.
...
Auf jedem Webserver könnte ein eigener RPC-Server laufen, welcher Requests nur an weitere RPC-Server leitet, jedoch die entsprechende TCP-Connectionen offenhält. Dies würde dazu führen, dass Nachrichten extrem schnell und ressourcenschonend weitergeleitet werden könnten. Die Connection zu dem lokalen RPC Server könnten sogar über UDP / SharedMemory / IPC gehändelt werden.
...
Letztlich ist eigentlich nur wichtig, dass es einen Client gibt, der einen RPC Request irgendwo hin schickt, schließlich ist seitens des Clients der Server in der Verantwortung eine Antwort zu schicken.

Es wäre also durchaus Möglich auf den eigentlichen Worker zu verzichten, bzw. Worker und Server zusammen zu legen:
...
Im Falle einer Programmiersprache wie PHP ergibt es vermutlich Sinn Sever und Worker zu trennen um mehr als ein CPU Kern zu belasten. Wichtig ist letztlich nur die Erkentniss, dass sich Server/Worker beliebig schachteln könnten.
...
Hier gibt es unterschiedliche Strategien. Ich bin hier noch am abwegen.
