...
Der Name der Klasse kommt ganz oben, gefolgt von beliebig viele Properties. Danach folgen die Funktionen.
...
Dieses Diagram sagt nchts anderes aus, dass ClassA Beziehung mit ClassB steht.
oft bedeutet dies das die eine Klasse eine Instanz der anderen Klasse hält dies ist aber nicht zwingend notwendig.


![](http://yuml.me/diagram/scruffy;/class/[Customer]->[Address])

Customer beinahltet eine Instanz eine Adresse. Vom Customer lässt sich auf die Adresse zugreifen.
Dies wäre z.b. für folgenen Code zulässig:
...
Dieses ist schlicht nicht bei jedem Kunden geben :).
...
Das Beispiel zeigt ein typisches Aggregate Root welches eine E-Mail, eine Adresse und optional ein Todes-Datum des Kunden hält.
...
Uml bietet unterschiedliche Möglichkeiten die Beziehungen zwischen Objekten

![](http://yuml.me/diagram/scruffy;/class/[Customer]1-n[Address])

Dies sagt schlicht aus, dass es mehrere Adressen in Beziehung zu einem Kunden stehen.

n kann für folgenes stehen:
...
Einfache Assoziationen lassen sich auch so darstellen, alternativ können "Assiziationen" hervorgehoben werden.
...
Dies bedetet das ein Verein aus mind 100 Mitgliedern besteht.
...
bei Quelified Associatons kann mit angegeben über welchen Wert (z.b. benötigter Parameter oder HashKey) eine Assoziation aufgebaut wird.
Dies wird durch einen kleineren eckigen Kasten neben der der Klasse dargestellt. Yuml unterstützt leider eine Qualified Associations, aus diesem Grund gibts hier kein Beispiel.
...
dass ein Entity nur für ein Aggrate Root bzw. ein Value Objekt nur für ein Entity eistiert.
...
Durch Abhängigkeiten signalisiert man dass wenn sich in einer etwas ändert, dass womöglich zu Änderungen in einer anderen Klasse führt.
